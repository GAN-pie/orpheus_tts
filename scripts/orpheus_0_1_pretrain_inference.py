...
"""
Orpheus-0.1-Pretrain-Inference.ipynb
"""
...
try:
  generated_ids = torch.cat([generated_ids, torch.tensor(generated_ids.size(0)*[[128262]]).to("cuda")], dim=1) # EOAI
except RuntimeError as err:
    print(err)
    print(generated_ids.size())
    print(torch.tensor([[128262]]).size())
    exit()
...
for i, code_list in enumerate(code_lists):
  ...
  torchaudio.save(f"cloning_test_{i}.wav", samples.squeeze(0), 22050)
